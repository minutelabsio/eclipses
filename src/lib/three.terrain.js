...
/**
 * THREE_Terrain.js 2.0.0-20220705
 *
 ...
 *
 * Usage: `var terrainScene = THREE_Terrain();`
 *
 ...
 *     as those for {@link THREE_Terrain.DiamondSquare}: an array of
 ...
 *     parameter for the `THREE_Terrain` function.
 ...
 *     include `THREE_Terrain.Linear` (the default), `THREE_Terrain.EaseIn`,
 *     `THREE_Terrain.EaseOut`, `THREE_Terrain.EaseInOut`,
 *     `THREE_Terrain.InEaseOut`, and any custom function that accepts a float
 ...
 *     `options.ySegments + 1`.
 */
const THREE_Terrain = function (options) {
  var defaultOptions = {
    after: null,
    easing: THREE_Terrain.Linear,
    heightmap: THREE_Terrain.DiamondSquare,
    material: null,
    maxHeight: 100,
    minHeight: -100,
    optimization: THREE_Terrain.NONE,
    frequency: 2.5,
    steps: 1,
    stretch: true,
    turbulent: false,
  ...
  var zs = THREE_Terrain.toArray1D(mesh.geometry.attributes.position.array);
  if (options.heightmap instanceof HTMLCanvasElement || options.heightmap instanceof Image) {
    THREE_Terrain.fromHeightmap(zs, options);
  }
  ...
  THREE_Terrain.fromArray1D(mesh.geometry.attributes.position.array, zs);
  THREE_Terrain.Normalize(mesh, options);
 ...
 *   displayed. Valid options are the same as for {@link THREE_Terrain}().
 */
THREE_Terrain.Normalize = function (mesh, options) {
  var zs = THREE_Terrain.toArray1D(mesh.geometry.attributes.position.array);
  if (options.turbulent) {
    THREE_Terrain.Turbulence(zs, options);
  }
  if (options.steps > 1) {
    THREE_Terrain.Step(zs, options.steps);
    THREE_Terrain.Smooth(zs, options);
  }

  // Keep the terrain within the allotted height range if necessary, and do easing.
  THREE_Terrain.Clamp(zs, options);
 ...
 *
 *          - `THREE_Terrain.NONE`: Don't apply any optimizations. This is the
 *            default.
 *          - `THREE_Terrain.GEOMIPMAP`: Divide the terrain into evenly-sized
 *            sections with multiple levels of detail. For each section,
 *            display a level of detail dependent on how close the camera is.
 *          - `THREE_Terrain.GEOCLIPMAP`: Divide the terrain into donut-shaped
 ...
 */
THREE_Terrain.NONE = 0;
THREE_Terrain.GEOMIPMAP = 1;
THREE_Terrain.GEOCLIPMAP = 2;
THREE_Terrain.POLYGONREDUCTION = 3;
 ...
 */
THREE_Terrain.toArray2D = function (vertices, options) {
 ...
 */
THREE_Terrain.fromArray2D = function (vertices, src) {
 ...
 */
THREE_Terrain.toArray1D = function (vertices) {
 ...
 */
THREE_Terrain.fromArray1D = function (vertices, src) {
 ...
 *
 * This is like {@link THREE_Terrain.toHeightmap} except that instead of
 ...
 *   {@link THREE_Terrain} function.
 * @param {Number} options
 *   The same as the options parameter for the {@link THREE_Terrain} function.
 */
THREE_Terrain.heightmapArray = function (method, options) {
  ...
  method(arr, options);
  THREE_Terrain.Clamp(arr, options);
  return arr;
};
 ...
 */
THREE_Terrain.Linear = function (x) {
  return x;
};

// x = [0, 1], x^2
THREE_Terrain.EaseIn = function (x) {
  return x * x;
};

// x = [0, 1], -x(x-2)
THREE_Terrain.EaseOut = function (x) {
  return -x * (x - 2);
};
...
THREE_Terrain.EaseInOut = function (x) {
  return x * x * (3 - 2 * x);
};

// x = [0, 1], 0.5*(2x-1)^3+0.5
THREE_Terrain.InEaseOut = function (x) {
  var y = 2 * x - 1;
  return 0.5 * y * y * y + 0.5;
};

// x = [0, 1], x^1.55
THREE_Terrain.EaseInWeak = function (x) {
  return Math.pow(x, 1.55);
};

// x = [0, 1], x^7
THREE_Terrain.EaseInStrong = function (x) {
 ...
 *   of {@link THREE_Terrain}().
 */
THREE_Terrain.fromHeightmap = function (g, options) {
 ...
 *
 * Parameters are the same as for {@link THREE_Terrain.fromHeightmap} except
 ...
 */
THREE_Terrain.toHeightmap = function (g, options) {
 ...
 *   A map of settings that control how the terrain is constructed and
 *   displayed. Valid values are the same as those for the `options` parameter
 *   of {@link THREE_Terrain}() but only `maxHeight`, `minHeight`, and `easing`
 *   are used.
 */
THREE_Terrain.Clamp = function (g, options) {
  var min = Infinity,
    max = -Infinity,
    l = g.length,
    i;
  options.easing = options.easing || THREE_Terrain.Linear;
  for (i = 0; i < l; i++) {
    if (g[i] < min) min = g[i];
    if (g[i] > max) max = g[i];
  }
 ...
 *   by this operation.
 * @param {Number/Function} [e=THREE_Terrain.EaseInOut]
 ...
 *   Valid values include `THREE_Terrain.Linear`, `THREE_Terrain.EaseIn`,
 *   `THREE_Terrain.EaseOut`, `THREE_Terrain.EaseInOut`,
 *   `THREE_Terrain.InEaseOut`, and any custom function that accepts a float
 *   between 0 and 1 and returns a float between 0 and 1.
 ...
 */
THREE_Terrain.Edges = function (g, options, direction, distance, easing, edges) {
  ...
  easing = easing || THREE_Terrain.EaseInOut;
 ...
 *   between 0 and 1 and returns a float between 0 and 1.
 */
THREE_Terrain.RadialEdges = function (g, options, direction, distance, easing) {
 ...
 *   neighbors.
 */
THREE_Terrain.Smooth = function (g, options, weight) {
 ...
 *   of {@link THREE_Terrain}().
 */
THREE_Terrain.SmoothMedian = function (g, options) {
  var heightmap = new Float32Array(g.length),
    neighborValues = [],
    neighborKeys = [],
 ...
 */
THREE_Terrain.SmoothConservative = function (g, options, multiplier) {
  var heightmap = new Float32Array(g.length);
  for (var i = 0, xl = options.xSegments + 1, yl = options.ySegments + 1; i < xl; i++) {
    for (var j = 0; j < yl; j++) {
      var max = -Infinity,
 ...
 *   (g.length/2)^(1/4).
 */
THREE_Terrain.Step = function (g, levels) {
 ...
 *   The geometry's z-positions to modify with heightmap data.
 * @param {Object} [options]
 *   The same map of settings you'd pass to {@link THREE_Terrain()}. Only
 *   `minHeight` and `maxHeight` are used (and required) here.
 */
THREE_Terrain.Turbulence = function (g, options) {
 ...
 */
THREE_Terrain.MultiPass = function (g, options, passes) {
  var clonedOptions = {};
 ...
 */
THREE_Terrain.Curve = function (g, options, curve) {
 ...
 *
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.Cosine = function (g, options) {
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.CosineLayers = function (g, options) {
  THREE_Terrain.MultiPass(g, options, [
    { method: THREE_Terrain.Cosine, frequency: 2.5 },
    { method: THREE_Terrain.Cosine, amplitude: 0.1, frequency: 12 },
    { method: THREE_Terrain.Cosine, amplitude: 0.05, frequency: 15 },
    { method: THREE_Terrain.Cosine, amplitude: 0.025, frequency: 20 },
  ]);
};
 ...
 *   of {@link THREE_Terrain}().
 */
THREE_Terrain.DiamondSquare = function (g, options) {
  ...
  // THREE_Terrain.SmoothConservative(g, options);
};
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.Fault = function (g, options) {
  ...
  // THREE_Terrain.Smooth(g, options);
};
 ...
 *   displayed. Valid values are the same as those for the `options` parameter
 *   of {@link THREE_Terrain}().
 * @param {Function} [feature=THREE_Terrain.Influences.Hill]
 ...
 *   `THREE_Terrain.Influences` contains some useful functions for this
 *   purpose.
 ...
 */
THREE_Terrain.Hill = function (g, options, feature, shape) {
  ...
  feature = feature || THREE_Terrain.Influences.Hill;
  ...
  for (var i = 0; i < numFeatures; i++) {
    ...
    THREE_Terrain.Influence(
      g, options,
      feature,
      coords.x, coords.y,
      radius, height,
      THREE.AdditiveBlending,
      THREE_Terrain.EaseInStrong
    );
  }
};
 ...
 *   purpose.
 */
THREE_Terrain.HillIsland = (function () {
  ...
  return function (g, options, feature) {
    THREE_Terrain.Hill(g, options, feature, island);
  };
})();
...
(function () {
   ...
   * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
   */
  THREE_Terrain.Particles = function (g, options) {
    ...
    // THREE_Terrain.Smooth(g, options, 3);
  };
})();
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.Perlin = function (g, options) {
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.PerlinDiamond = function (g, options) {
  THREE_Terrain.MultiPass(g, options, [
    { method: THREE_Terrain.Perlin },
    { method: THREE_Terrain.DiamondSquare, amplitude: 0.75 },
    { method: function (g, o) { return THREE_Terrain.SmoothMedian(g, o); } },
  ]);
};
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.PerlinLayers = function (g, options) {
  THREE_Terrain.MultiPass(g, options, [
    { method: THREE_Terrain.Perlin, frequency: 1.25 },
    { method: THREE_Terrain.Perlin, amplitude: 0.05, frequency: 2.5 },
    { method: THREE_Terrain.Perlin, amplitude: 0.35, frequency: 5 },
    { method: THREE_Terrain.Perlin, amplitude: 0.15, frequency: 10 },
  ]);
};
 ...
 */
THREE_Terrain.Simplex = function (g, options) {
  noise.seed(Math.random());
  var range = (options.maxHeight - options.minHeight) * 0.5,
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.SimplexLayers = function (g, options) {
  THREE_Terrain.MultiPass(g, options, [
    { method: THREE_Terrain.Simplex, frequency: 1.25 },
    { method: THREE_Terrain.Simplex, amplitude: 0.5, frequency: 2.5 },
    { method: THREE_Terrain.Simplex, amplitude: 0.25, frequency: 5 },
    { method: THREE_Terrain.Simplex, amplitude: 0.125, frequency: 10 },
    { method: THREE_Terrain.Simplex, amplitude: 0.0625, frequency: 20 },
  ]);
};
...
(function () {
   ...
   * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
   */
  THREE_Terrain.Value = function (g, options) {
    ...
    // THREE_Terrain.Smooth(g, options, 1);
    THREE_Terrain.Clamp(g, {
      maxHeight: options.maxHeight,
      minHeight: options.minHeight,
      stretch: true,
 ...
 * Parameters are the same as those for {@link THREE_Terrain.DiamondSquare}.
 */
THREE_Terrain.Weierstrass = function (g, options) {
  var range = (options.maxHeight - options.minHeight) * 0.5,
 ...
 *    var material = THREE_Terrain.generateBlendedMaterial([
 ...
 *   `MeshLambertMaterial`.
 */
THREE_Terrain.generateBlendedMaterial = function (textures, material) {
 ...
 *     `THREE_Terrain()` or added to that return value; otherwise the position
 ...
 *   return value of a call to `THREE_Terrain()` or added to that return value;
 *   otherwise the position and rotation of the meshes will be wrong.
 */
THREE_Terrain.ScatterMeshes = function (geometry, options) {
  if (!options.mesh) {
    console.error('options.mesh is required for THREE_Terrain.ScatterMeshes but was not passed');
    return;
  }
  if (!options.scene) {
  ...
  for (var i = 0; i < geometry.attributes.position.array.length; i += 9) {
    ...
    if (spreadIsNumber) {
      ...
      if (rv < options.spread) {
        ...
        place = THREE_Terrain.EaseInOut((rv - options.spread) * spreadRange) * options.spread > Math.random();
      }
    }
    else {
 ...
 *   `options.heightmap` parameter of the `THREE_Terrain` function).
 * @param {Object} options
 ...
 *   `THREE_Terrain` function). `options.minHeight` must equal `0` and
 ...
 *   `options.randomness` parameter to the {@link THREE_Terrain.ScatterMeshes}
 *   function.
 */
THREE_Terrain.ScatterHelper = function (method, options, skip, threshold) {
  skip = skip || 1;
  ...
  var heightmap = THREE_Terrain.heightmapArray(method, clonedOptions);
 ...
 */
THREE_Terrain.Influences = {
  Mesa: function (x) {
    return 1.25 * Math.min(0.8, Math.exp(-(x * x)));
  },
  Hole: function (x) {
    return -THREE_Terrain.Influences.Mesa(x);
  },
  Hill: function (x) {
  ...
  Valley: function (x) {
    return -THREE_Terrain.Influences.Hill(x);
  },
  Dome: function (x) {
    // Parabola
    return -(x + 1) * (x - 1);
 ...
 * @param {Number/Function} [e=THREE_Terrain.EaseIn]
 ...
 *   interpolating the height of each vertex along a curve. Valid values
 *   include `THREE_Terrain.Linear`, `THREE_Terrain.EaseIn`,
 *   `THREE_Terrain.EaseOut`, `THREE_Terrain.EaseInOut`,
 *   `THREE_Terrain.InEaseOut`, and any custom function that accepts a float
 ...
 */
THREE_Terrain.Influence = function (g, options, f, x, y, r, h, t, e) {
  f = f || THREE_Terrain.Influences.Hill; // feature shape
  ...
  e = e || THREE_Terrain.EaseIn; // falloff
